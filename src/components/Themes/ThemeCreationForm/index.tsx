...
const ThemeCreationForm: React.FC<ThemeCreationForm> = ({ onThemeCreate }) => {
  ...
  return (
<form
  className="bg-gray-800 p-4 rounded-lg shadow-lg" // Diminui o padding
  onSubmit={handleSubmit}
>
  <div>
    <label htmlFor="title" className="text-sm font-semibold">Assunto:</label>
    <input
  ...
  <div className="mt-4"> {/* Adiciona um espaço entre os campos */}
    <label htmlFor="keywords" className="text-sm font-semibold">Palavras-chave:</label>
    <input
      className="mt-2 block w-full bg-gray-700 border border-gray-600 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" // Ajusta a margin e estilo
      id="keywords"
      type="text"
  ...
  <div className="flex justify-end mt-4"> {/* Mantém a margin do botão */}
    <button
